...
export class newPlaylistMenu extends bottomSheetMenu {
  ...
  async customActivitesSelection() {
    ...
    const hideList = [this.nameInput, this.cancel, this.doneButton, this.addButton, this.deleteButton, this.playlistDurationText, this.excerciseList];
    const enableList = [this.backButton];
    this.cleanContent();
    ...
    for (const item of Object.keys(customActivties)) {
  ...
  async fileinputHandler(urlReader) {
    try {
      ...
      for (const activity of Object.keys(file.activities)) {
  ...
  async exportPlaylist() {
    ...
    const downloadJSON = {};
    const playlist = await fetch(`playlist/${this.UUID}`).then(res => res.json());
    const activities = {};
    downloadJSON.playlist = playlist;
    for (const activity of playlist.activites) {
