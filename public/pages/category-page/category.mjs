...
async function isActivitiesEmpty(exercises) {
  ...
  for (const item of Object.keys(exercises)) {
...
export async function displayCategoryPage() {
  ...
  prepareHandles();
  const empty = isLocalStorageEmpty(TAG_KEY);
  ...
  if (empty !== undefined || empty !== null) {
    console.log("esdfsdfsd", JSON.parse(localStorage.getItem(TAG_KEY)));
    const tagJSON = JSON.parse(localStorage.getItem(TAG_KEY));
    for (const key of Object.keys(tagJSON)) {
      const tagSelect = await fetchFragment(import.meta.resolve('./category-item.inc'));
      tagSelect.textContent = key;
      tagSelect.addEventListener('click', async () => { await getTagActivites(key, tagJSON[key].activites); });
